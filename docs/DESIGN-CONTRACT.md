...
This is not a "show off the model's intelligence" demo. The skill MUST be simple enough that a 26B local model executes it cleanly. If a step is too subtle for gemma-26b, it is too subtle for the skill — simplify the script, not the model.
...
1. **What's the current shippable target?** (Today: gemma-4-26b-64k via Telegram. NOT a cloud LLM.)
