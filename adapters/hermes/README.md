...
| `tools/form_tool.py` | LLM-facing tool definition. Registers `name="form"` via `tools.registry.registry.register(...)` (Hermes' own plugin entry point — anything dropped in `tools/` is auto-discovered). Registered under the **`clarify` toolset**, not a novel one: `get_tool_definitions()` is a per-toolset allowlist and the static `hermes-<platform>` composites don't resolve runtime-registered toolset names, so a tool under its own toolset is never offered to any platform agent. Riding `clarify` (present in every composite; form is its multi-field sibling) is what makes the tool reachable. `U1_FORM_TOOLSET` overrides if a Hermes build grows a first-party form toolset. Schema description + thin dispatcher to a platform-provided callback. Also carries the class-level monkey-patch of the Telegram platform class (`send_form` + form callback routing — no Hermes source edits). Version-adaptive: resolves `plugins.platforms.telegram.adapter.TelegramAdapter` on hermes-agent ≥ 0.18 (the plugin-adapter refactor), falling back to `gateway.platforms.telegram.TelegramPlatform` on ≤ 0.17; if neither imports it logs every path it tried and degrades to text fallback. |
