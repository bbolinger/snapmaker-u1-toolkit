...
| `tools/form_tool.py` | LLM-facing tool definition. Registers `name="form"` via `tools.registry.registry.register(...)` (Hermes' own plugin entry point — anything dropped in `tools/` is auto-discovered). Registration alone doesn't surface the tool: platform agents get a per-toolset allowlist, and on bare-composite configs (`platform_toolsets.telegram: [hermes-telegram]`) Hermes enables a toolset only when it's a **subset** of the composite — a runtime-registered toolset never qualifies, and joining an existing toolset poisons it (form in `clarify` makes `{clarify, form} ⊄ hermes-telegram`, evicting clarify itself). So `form` keeps its own toolset and the module injects it **directly into `_get_platform_tools`' result** at import — the same escape hatch Hermes uses for `x_search`. Unknown module layouts or unexpected result shapes log a warning and leave Hermes' resolution untouched. Schema description + thin dispatcher to a platform-provided callback. Also carries the class-level monkey-patch of the Telegram platform class (`send_form` + form callback routing — no Hermes source edits). Version-adaptive: resolves `plugins.platforms.telegram.adapter.TelegramAdapter` on hermes-agent ≥ 0.18 (the plugin-adapter refactor), falling back to `gateway.platforms.telegram.TelegramPlatform` on ≤ 0.17; if neither imports it logs every path it tried and degrades to text fallback. |
