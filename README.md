...
profiles extracted from prints that already succeeded on your printer, and
your own hand-tuned files. The full guide (first-run setup, supports
auto-detection, and building per-extruder profiles from print history) is in
[docs/PROFILES.md](docs/PROFILES.md).

## Headless slicing setup (no GUI / scripted)

Slicing from the CLI in a container, CI pipeline, or agent workflow, with no
OrcaSlicer GUI: which build to use, the 3-profile load order, a real
inheritance pitfall and its workaround, and the orientation-review and
thumbnail-injection tools. Full guide: [docs/HEADLESS.md](docs/HEADLESS.md).
