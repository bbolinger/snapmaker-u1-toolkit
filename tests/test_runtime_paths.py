...
def test_run_workflow_threads_session_key_into_child_env(monkeypatch):
    """Regression (live 2026-07-12): the fresh-kit workflow is spawned by this
    custom tool via subprocess.run, which inherited the gateway's os.environ —
    and the gateway keeps HERMES_SESSION_KEY in a contextvar, NOT its env. So
    the child was blind to it and _arm_pending_attach refused to arm the
    structural-attachment marker; images fell back to the model echoing paths
    (one gemma-mangled → only 2 of 3 photos). The tool must thread the session
    key it already holds into the child env."""
    tool = _load_kit_tool()
    captured = {}
    ...
    tool._run_workflow(["python", "wf.py"], timeout=10,
                       session_key="telegram:8131922235:main")
    assert captured["env"] is not None, "must pass an explicit env, not inherit"
    assert captured["env"].get("HERMES_SESSION_KEY") == "telegram:8131922235:main"


def test_run_workflow_no_session_key_omits_it(monkeypatch):
    tool = _load_kit_tool()
    captured = {}
    ...
    tool._run_workflow(["python", "wf.py"], timeout=10)
    assert "HERMES_SESSION_KEY" not in (captured["env"] or {})
